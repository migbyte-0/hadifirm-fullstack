...
let gsap,
  ...
  _parseAnimation = (animationOrId) =>
    animationOrId instanceof Animation
      ? animationOrId
      : animationOrId
      ? gsap.getById(animationOrId)
      : null,
  _createElement = (type, container, cssText) => {
    let element = _doc.createElementNS
      ? _doc.createElementNS(type === "svg" ? _svgNS : _domNS, type)
  ...
  _getClippedDuration = (animation, excludeRootRepeats) => {
    ...
    while (t) {
      max = Math.max(
        max,
        t.totalDuration() > 999 ? t.endTime(false) : t._start + t._tDur / t._ts
      );
      t = t._next;
    }
    return !excludeRootRepeats && repeat
      ? max * (repeat + 1) + animation._rDelay * repeat
      : max;
  },
  ...
  _createRootElement = (element, minimal, css) => {
    if (!_addedCSS) {
      _createElement("style", _docEl).innerHTML =
  ...
  _addListener = (e, type, callback, capture) => {
    ...
    if (type === "mousedown") {
      ...
      altType = !_isUndefined(e.onpointerdown)
        ? "pointerdown"
        : !_isUndefined(e.ontouchstart)
        ? "touchstart"
        : null;
      if (altType) {
        handler = (event) => {
  ...
  _selectValue = (element, value, label, insertIfAbsent) => {
    ...
    if (insertIfAbsent) {
      ...
      option.innerHTML = label.innerHTML = _isString(insertIfAbsent)
        ? insertIfAbsent
        : value;
  ...
  _merge = () => {
    ...
    if (_rootInstance) {
      ...
      while (t) {
        ...
        if (
          !(_isFunction(target) && target === t.vars.onComplete && !t._dur) &&
          !(target && target._gsIgnore)
        ) {
  ...
  _buildPlayPauseMorph = (svg) => {
    let tl = gsap.timeline(
      { data: "root", parent: _independentRoot, onComplete: () => tl.kill() },
  ...
  _buildLoopAnimation = (svg) => {
    ...
    tl.to(svg, {
      ...
      svg.querySelectorAll(".loop-path"),
  ...
  _getAnimationById = (id) =>
    gsap.getById(id) ||
    _independentRoot.getById(id) ||
    (id === _recordedRoot.vars.id && _recordedRoot),
  _initCore = (core) => {
    gsap = core || _getGSAP();
    if (!_coreInitted) {
      if (gsap && _windowExists()) {
        ...
        _globalTimeline.killTweensOf = function (targets, props, onlyActive) {
          ...
          _recordedRoot.killTweensOf.call(
            _globalTimeline,
            targets,
            props,
            onlyActive
        ...
        _recordedRoot._start =
          _globalTimeline._time =
          _globalTimeline._tTime =
            0;
        _delayedCall = (delay, callback, params, scope) =>
          gsap.to(
            callback,
            {
              delay: delay,
        ...
        _delayedCall(0.01, () =>
          _rootInstance ? _rootInstance.update() : _merge()
        );
  ...
  GSDevTools = function (vars) {
    ...
    let _self = this,
      ...
      recall = (key) => {
        ...
        if (vars.persist !== false && _supportsStorage) {
          ...
          return key === "animation"
            ...
            : key === "loop"
            ? value === "true"
            : parseFloat(value); // handle data typing too.
        }
      },
      //SCRUBBER/PROGRESS
        ...
        onDrag: function () {
          ...
          progressBar.style.width =
            Math.min(
              ...
              Math.max(0, (time / linkedAnimation._dur) * 100 - inProgress)
            ) + "%";
      ...
      updateProgress = function (force) {
        ...
        let p =
            !loopEnabled && selectedAnimation._repeat === -1
              ? (selectedAnimation.totalTime() / selectedAnimation.duration()) *
                100
              : linkedAnimation.progress() * 100 || 0,
          repeatDelayPhase =
            selectedAnimation._repeat &&
            selectedAnimation._rDelay &&
            selectedAnimation.totalTime() %
              (selectedAnimation.duration() + selectedAnimation._rDelay) >
      ...
      loop = (value) => {
        ...
        if (loopEnabled) {
          ...
          if (linkedAnimation.progress() >= outProgress / 100) {
            let target =
              linkedAnimation._targets && linkedAnimation._targets[0];
            if (target === selectedAnimation) {
              //in case there are callbacks on the timeline, when we jump back to the start we should seek() so that the playhead doesn't drag [backward] past those and trigger them.
              target.seek(
                startTime + ((endTime - startTime) * inProgress) / 100
      ...
      //ANIMATIONS list
      updateList = () => {
        let animations = _getChildrenOf(
            declaredAnimation && !vars.globalSync
              ? declaredAnimation
              : _recordedRoot,
            true
          ),
      ...
      animation = function (anim) {
        ...
        if (linkedAnimation) {
          ...
          if (
            linkedAnimation._targets &&
            linkedAnimation._targets[0] === declaredAnimation
          ) {
        ...
        maxDuration =
          vars.maxDuration ||
          Math.min(1000, _getClippedDuration(selectedAnimation));
        if (selectedAnimation === _recordedRoot || vars.globalSync) {
          ...
          startTime = Math.min(
            inProgress * selectedAnimation.duration(),
            selectedAnimation.time()
          );
      ...
      initialize = (preliminary) => {
        ...
        inProgress =
          ("inTime" in vars
            ? _timeToProgress(vars.inTime, selectedAnimation, 0, 0)
            : savedInOut
            ? savedAnimation._inProgress
            : 0) || 0;
    ...
    inPoint._gsIgnore =
      outPoint._gsIgnore =
      playhead._gsIgnore =
      playPauseButton._gsIgnore =
      loopButton._gsIgnore =
        true;
