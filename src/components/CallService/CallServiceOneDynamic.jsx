import { useCallService } from "@/context/HomeContentContext";
import PhonePlusYellow from "@/assets/images/icons/phone-plus-yellow.svg";
import { Link } from "react-router-dom";

export default function CallServiceOneDynamic() {
  const { text, phoneNumber, phoneDisplay, icon, loading } = useCallService();
        ...
        <div className="call-service px-3">
          <div className="col-12 col-md-10 col-lg-10 col-xl-10 flex-column flex-md-row m-auto d-flex justify-content-start justify-content-md-center align-items-centerc gap-4">
            ...
            <div
              ...
              data-delay="0.4"
            >
              <img className="call-icon" src={icon || PhonePlusYellow} alt="Icon" />
              <Link className="call-link" to={`tel:${phoneNumber}`}>
                {phoneDisplay}
              </Link>
            </div>
          </div>
